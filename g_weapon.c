...
static void fire_lead (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int te_impact, int hspread, int vspread, int mod)
{
	...
	if (!(tr.fraction < 1.0))
	{
		...
		{
			// angle changes chances of passing through alpha surface
			float p = (sqrt(fabs(DotProduct(aimdir, tr.plane.normal))) + 0.9) / 2;
			if (crandom() < p)
			{
...
//				VectorMA (alpha_start, 8192, forward, end);
		...
		if (!leadsound)
		{
			if (tr.surface->flags & SURF_METAL)
			{
				// TE_METAL_SPARKS plays weapons/ric1.wav 50% of time, so skip that here
				int r = rand() & 3;
				if (r >= 2)
					leadsound = gi.soundindex (va("weapons/ric%d.wav", r));
			}
			else if ((tr.surface->flags & (SURF_METAL_L | SURF_SNOW)) && !(tr.surface->flags & SURF_ALPHA)) // SURF_SNOW = tin
			{
				switch (rand()%3)
				{
					case 0:
						leadsound = gi.soundindex ("weapons/bullethit_tin.wav");
						break;
					case 1:
						leadsound = gi.soundindex ("weapons/bullethit_tin2.wav");
						break;
					case 2:
						leadsound = gi.soundindex ("weapons/bullethit_tin3.wav");
						break;
				}
			}
			else if (tr.surface->flags & SURF_WOOD)
			{
				leadsound = gi.soundindex (va("weapons/bullethit_wood%d.wav", 1 + (rand()%3)));
			}
			// svc_temp_entity & svc_muzzleflash sounds are only sent to PVS, so do the same with these
			if (leadsound)
				gi.positioned_sound (tr.endpos, g_edicts, CHAN_AUTO | CHAN_PVS, leadsound, 1, ATTN_NORM, 0);
		}
...
//						if (tr.surface->flags & (SURF_METAL | SURF_METAL_L))
						if ((tr.surface->flags & SURF_METAL) || ((tr.surface->flags & SURF_METAL_L) && (tr.surface->flags & SURF_ALPHA)))
						{
							te_impact = TE_METAL_SPARKS;
							ClipVelocity( aimdir, tr.plane.normal, dir, 1.5 );
						}
...
/*	if (self->client)
	...
	grenade->s.effects |= EF_GRENADE | EF_ROTATE;
	VectorSet(grenade->mins, -2, -2, -2);
	VectorSet(grenade->maxs, 2, 2, 2);
	
	grenade->s.modelindex = gi.modelindex ("models/weapons/v_grenade/thrown.mdx");
...
void fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	rocket->s.sound = gi.soundindex ("weapons/rocket_launcher/rl_rocket.wav");
	...
	{
		rocket->launch_delay = self->client->currentping / 2 // network delay (assuming each way takes half of ping time)
...
void FlameJunc_Think( edict_t *self )
{
	...
	{	// copied from ClipVelocity()
		...
		if ((tr.contents & MASK_ALPHA) || (tr.ent && (tr.ent->s.renderfx2 & RF2_SURF_ALPHA)))
		{
			...
			while ((dist < maxdist) && tr.startsolid && ((tr.contents & MASK_ALPHA) || (tr.ent && (tr.ent->s.renderfx2 & RF2_SURF_ALPHA))))
			{
		...
		backoff = DotProduct (self->velocity, tr.plane.normal) * 1.01; // slide
...
/*	VectorCopy( self->s.origin, org );
	...
	VectorCopy(self->s.origin, org);
	VectorMA(org, 7, xright, org);
	org[2] += self->viewheight - 2;

	// check for self damage from close surface
	if (!(self->svflags & SVF_MONSTER))
	{
		VectorCopy(org, start);
		VectorMA(start, FLAME_START_OFFSET + 32, forward, end);
		tr = gi.trace(start, mins, maxs, end, self, MASK_SOLID);
		if (tr.fraction < 1)
		{
			if ((tr.contents & MASK_ALPHA) || (tr.ent && (tr.ent->s.renderfx2 & RF2_SURF_ALPHA)))
			{
				VectorMA(tr.endpos, 0.5, forward, start);
				tr = gi.trace(start, mins, maxs, end, self, MASK_SOLID);
				if (tr.fraction == 1)
					goto skip_selfdamage;
			}
			if (tr.ent->takedamage)
...
/*	VectorMA( junc->s.origin, 0, xup, junc->s.origin );
	VectorMA( junc->s.origin, 4, xright, junc->s.origin );*/
	...
	tr = gi.trace ( oldorg, jmins, jmaxs, junc->s.origin, self, MASK_PLAYERSOLID );
