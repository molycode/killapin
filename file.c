...
qboolean LoadConfig()
{
	...
	default_timelimit[0] = 0;
	default_fraglimit[0] = 0;
	...
	while (fgetline(infile, buffer))	// Retrieve line from the file
	{
		...
		else if (!strcmp(key, "default_teams"))
		{
			if (c == 2)
				strncpy(default_teams, param, 16);
		}
		...
		else if (!strcmp(key, "pregame_time"))
		{
			if (c == 2)
	...
	if (map_list_filename[0])
	{
		...
		if (infile != NULL)
		{
			while (fgetline(infile, buffer))	// Retrieve line from the file
			{
				...
				if (!CheckFileExists(va("maps/%s.bsp", maplist[num_maps])))
				{
					if (kpded2)
					{
						// check for an "override" file
						if (CheckFileExists(va("maps/%s.bsp.override", maplist[num_maps])))
							goto mapok;
					}
