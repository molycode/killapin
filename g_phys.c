...
//retry:
	...
	if (part)
	{
		...
		// gib dead players
		if (obstacle->health <= 0 && (obstacle->client || (obstacle->classname && !strcmp(obstacle->classname, "bodyque"))))
		{
			obstacle->health -= 100;
			obstacle->die(obstacle, part, part, 100, obstacle->s.origin, 0, 0);
		}
		else
...
// regular thinking
	SV_RunThink (ent);
	if (!ent->inuse) // got freed during think
		return;
...
// move origin
	...
	if (trace.fraction < 1)
	{
		// remove dropped items that touch sky/space
		if ((ent->spawnflags & (DROPPED_ITEM|DROPPED_PLAYER_ITEM)) && trace.surface && (trace.surface->flags & SURF_SKY))
		{
			G_FreeEdict(ent);
			return;
		}
	...
	// stop if on ground
		...
		{		
			if (ent->velocity[2] < 60 || ent->movetype != MOVETYPE_BOUNCE )
			{
				...
				if (ent->classname && !strcmp(ent->classname, "grenade"))
					ent->s.angles[PITCH] = 0; // fall onto side
			}
		}
...
// check for water transition
	...
	// END JOSEPH
	if (!wasinwater && isinwater)
		gi.positioned_sound (old_origin, g_edicts, CHAN_AUTO | CHAN_PVS, gi.soundindex("actors/player/step water/watr_in.wav"), 0.5, ATTN_NORM, 0);
