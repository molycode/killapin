...
qboolean Pickup_AmmoBox (edict_t *ent, edict_t *other)
{
	return Add_Ammo(other, FindItem("308cal"), 30)
		| Add_Ammo(other, FindItem("Bullets"), 50)
		| Add_Ammo(other, FindItem("Shells"), 8);
}
...
void Touch_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (plane && (ent->flags & FL_IMPACTSOUND))
	{
		int footsteptype;

		if (plane->normal[2] < 0.5 || -ent->velocity[2] < 100)
			return;

		// waterlevel is updated after touches, so need to check that here
		if (ent->waterlevel || (gi.pointcontents(ent->s.origin) & MASK_WATER))
			return;
		...
		BodyImpactSound(ent, footsteptype, 250);
		return;
	}
	...
	// bosses can only pickup health
	if (other->client->resp.is_boss && Q_stricmp(ent->item->pickup_name, "Large Health"))
		return;
...
edict_t *Drop_Item (edict_t *ent, gitem_t *item)
{
	...
	if (ent->client)
	{
		...
		angle[PITCH] = angle[PITCH] * 0.75;
		AngleVectors (angle, forward, right, NULL);
		VectorSet(offset, 28, 0, ent->viewheight - 4);
	...
	if (ent->client)
	{
		VectorScale (forward, 200, dropped->velocity);
...
void SpawnItem (edict_t *ent, gitem_t *item)
{
	...
	// no pickups
	if ((item->flags & IT_WEAPON) || (item->flags & IT_AMMO) || item->pickup == Pickup_Pistol_Mods || item->pickup == Pickup_Pack || item->pickup == Pickup_Armor
		|| !strcmp(item->classname, "item_health_sm") || !strcmp(item->classname, "item_health_lg") || !strcmp(item->classname, "item_adrenaline"))
	{
		G_FreeEdict (ent);
		return;
	}
...
/* precache */ ""
...

	{
		...
		Weapon_Grenade,
		"misc/w_pkup.wav",
		"models/weapons/g_grenade/tris.md2", EF_REFL,
		"models/weapons/v_grenade/v_grenade.mdx",
/* icon */		"/pics/h_grenade.tga",
/* pickup */	"Grenade",
		0,
		1,
		...
		NULL,
		0,
/* precache */ "weapons/grenade/pin.wav"  // snap, this is the fire sound for the grenade
	},
#if 0
	{
		"item_ammobox",
		...
		NULL,
		"world/pickups/ammo.wav",
		"models/pu_icon/ammo_box/box_tsh.md2", 0,
		NULL,
/* icon */		"/pics/h_tclip.tga",
/* pickup */	"Ammo Box",
/* width */		2,
...
/* precache */ ""
...

	{
		...
		"misc/w_pkup.wav",
		NULL, 0,
		"models/weapons/v_ammo/ammo.mdx",
/* icon */		"/pics/h_tclip.tga",
/* pickup */	"AmmoDrop",
		0,
		0,
...
/* precache */ "" 
...

	{
		...
		"misc/w_pkup.wav",
		NULL, 0,
		"models/weapons/v_health/health.mdx",
/* icon */		"/pics/h_lmed.tga",
/* pickup */	"HealthDrop",
		0,
		0,
		NULL,
...
#endif
//CDEATH
/* weapon_harpoon (.3 .3 1) (-16 -16 -16) (16 16 16)
*/
	{
		"weapon_harpoon", 
		Pickup_Weapon,
		Use_Weapon,
		Drop_Weapon,
		Harpoon_Weapon_Execute,
		"misc/w_pkup.wav",
		"models/weapons/g_harpoon/g_harpoon.md2", EF_REFL,
		"models/weapons/v_harpoon/v_harpoon.mdx",
/* icon */		"/pics/hh_harpoon.tga",
/* pickup */	"Harpoon",
		0,
		0,
		NULL,
		...
		NULL,
		0,
/* precache */ "weapons/harpoon/fire.wav weapons/harpoon/pull.wav weapons/harpoon/hang.wav weapons/harpoon/reset.wav weapons/harpoon/hit.wav"
	},
//END CDEATH
