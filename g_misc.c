...
void GibEntity( edict_t *self, edict_t *inflictor, float damage )
{
	...
	for (n= 0; n < 6; n++)
	{
		// put first one straight below
		if (!n)
			VectorSet(vel, 0, 0, self->mins[2] - 50);
		else
		{
			VelocityForDamage (damage, vel);
			VectorScale(vel, 0.5, vel);
			VectorMA(vel, 150, dir, vel);
		}
...
void AddLightSource(edict_t *self)
{
	...
	{
		gi.dprintf("light color not set at %s, discarding\n", vtos(self->s.origin));
		return;
	}
...
//			"%5i %5i %5i %1.1f %1.1f %1.1f %4i %3i",		// "99999 99999 99999 9.9 9.9 9.9 9999 999"
		...
		gi.configstring (CS_JUNIORS + i, str );
	}
}
...
// trivia reject 
		...
		intensity = level.light_values[i] - VectorLength(vec);
		if (intensity < 0)
			continue;

		intensity *= (level.light_colors[i][0] + level.light_colors[i][1] + level.light_colors[i][2]) / 3;
		if (ent_lights == 5 && intensity < intensities[4])
			continue;
...

		{
			...
			spot1[2] += self->maxs[2];
			trace = gi.trace (level.light_orgs[i], vec3_origin, vec3_origin, spot1, self, MASK_OPAQUE);
...
// passed normal tests. if we've used all lights, look for one to replace
		...
		for (j=0; j<ent_lights; j++)
		{
			if (intensities[j] < intensity)
			{
				...
				{
					// shift dimmer lights
					memmove(indexes + j + 1, indexes + j, (4 - j) * sizeof(int));
					memmove(intensities + j + 1, intensities + j, (4 - j) * sizeof(float));
				}
	...
	if (ent_lights && self->client)
	{
		if (intensities[0] < 30)
			ent_lights = 0; // too dark
	}

	if (!ent_lights && self->s.model_lighting.num_dir_lights)
	{
		i = self->s.model_lighting.light_indexes[0];
		VectorSubtract( level.light_orgs[i], self->s.origin, vec );
		intensity = level.light_values[i] - VectorLength(vec);
		if (intensity > 200)
			return; // keep existing lighting
	}

	if (ent_lights > MAX_MODEL_DIR_LIGHTS)
	{
		// try to have some backlight with directional lighting enabled even if not brightest
		if (self->s.renderfx2 & RF2_DIR_LIGHTS)
		{
			float angle0, angle;
			VectorSubtract( level.light_orgs[indexes[0]], self->s.origin, vec );
			angle0 = atan2(vec[1], vec[0]);
			for (j=1; j<ent_lights; j++)
			{
				i = indexes[j];
				VectorSubtract( level.light_orgs[i], self->s.origin, vec );
				angle = atan2(vec[1], vec[0]) - angle0;
				if (angle < 0)
					angle += 2 * M_PI;
				if (angle > M_PI * 0.7 && angle < M_PI * 1.3)
				{
					if (j >= MAX_MODEL_DIR_LIGHTS)
						indexes[MAX_MODEL_DIR_LIGHTS - 1] = i;
					break;
				}
			}
		}
		ent_lights = MAX_MODEL_DIR_LIGHTS;
	}
	...
	for (j=0; j<ent_lights; j++)
	{
		...
		if (showlights->value && self->client)
...
//CDEATH - self is teleporter entity, other is the player
	Harpoon_Player_Reset(other);
//END CDEATH
