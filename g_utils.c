...
// END:		Xatrix/Ridah/Navigator/19-mar-1998
	...
	e->s.last_lighting_update_pos[0] = 9999; // trigger directional lighting update
}
									...
									edict_t *impact_ent, vec3_t pos, vec3_t normal )
{
	...
	// randomise sprite rotation on floors/ceilings
	if (!normal[0] && !normal[1])
	{
		angles[PITCH] = (normal[2] > 0 ? -90 : -270);
		angles[YAW] = rand() % 360;
	}
	else
